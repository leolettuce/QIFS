...
def single_time_step(U, V, Ax_MPS, Ay_MPS, Bx_MPS, By_MPS, chi_mpo, dt, Re, mu, d1x, d1y, d2x, d2y, d1x_d1x, d1x_d1y, d1y_d1y, U_d1x_d1x_U_left, U_d1x_d1x_U_right, U_d1x_d1y_V_left, U_d1x_d1y_V_right, V_d1y_d1y_V_left, V_d1y_d1y_V_right, solver='cg', options=None):
    
    # initialize precontracted left and right networks
    U_Ax_left, U_Ax_right = get_precontracted_LR_mps_mps(U, Ax_MPS, 0, options)
    V_Ay_left, V_Ay_right = get_precontracted_LR_mps_mps(V, Ay_MPS, 0, options)
    
    n = len(U) # size of MPS

    # create MPOs for convection-diffusion terms
    mult_algorithm = {'qr_method': False, 'svd_method': {'partition': 'V', 'max_extent': chi_mpo}}
    ...
    while np.abs((E_1-E_0)/E_0) > epsilon:      # do until the state does not change anymore
        ...
        for i in range(n-1):      
            ...
            if solver == 'cg':
                ...
                H_22 = mu*dt**2 * left_right_W(V_d1y_d1y_V_left[i], V_d1y_d1y_V_right[i], d1y_d1y[i], options)
...
def time_evolution(U, V, chi_mpo, dt, T, Re, mu, save_path, options=None, solver='cg'):
    ...
    mult_algorithm = {'qr_method': False, 'svd_method': {'partition': 'V', 'max_extent': chi_mpo}} # 'rel_cutoff':1e-10, 
    ...
    for step in range(n_steps):   # for every time step dt
        ...
        # Midpoint RK-2 step
        U_mid, V_mid = single_time_step(U_trial, V_trial, U_prev, V_prev, U_prev_copy, V_prev_copy, chi_mpo, dt/2, Re, mu, d1x, d1y, d2x, d2y, d1x_d1x, d1x_d1y, d1y_d1y, U_d1x_d1x_U_left, U_d1x_d1x_U_right, U_d1x_d1y_V_left, U_d1x_d1y_V_right, V_d1y_d1y_V_left, V_d1y_d1y_V_right, solver=solver, options=options)
        # Full RK-2 step
        print('')
        U, V = single_time_step(U_trial, V_trial, U_prev, V_prev, U_mid, V_mid, chi_mpo, dt, Re, mu, d1x, d1y, d2x, d2y, d1x_d1x, d1x_d1y, d1y_d1y, U_d1x_d1x_U_left, U_d1x_d1x_U_right, U_d1x_d1y_V_left, U_d1x_d1y_V_right, V_d1y_d1y_V_left, V_d1y_d1y_V_right, solver=solver, options=options)
...
def build_initial_fields(n_bits, L, chi, y_min=0.4, y_max=0.6, h=1/200, u_max=1):
    ...
    MPS_U_cupy= convert_MPS_to_cupy(MPS_U, 4)
    MPS_V_cupy = convert_MPS_to_cupy(MPS_V, 4)

    return MPS_U_cupy, MPS_V_cupy, MPS_U, MPS_V